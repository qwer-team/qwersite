...
class DefaultController extends ControllerHelper
{
    ...
    public function indexAction()
    {
        ...
        $queryBuilder = $em->getRepository('ItcAdminBundle:Gallery\Image')
                ->createQueryBuilder('M')
                ->select('M, T')
                ->innerJoin('M.gallery', 'G', 'WITH', "G.menu = :menu ")
                ->leftJoin('M.translations', 'T', 'WITH', "T.locale = :locale")
        ...
        $queryBuilder = $em->getRepository('ItcAdminBundle:Menu\Menu')
                ...
                ->innerJoin('P.parent', 'PP', 'WITH', "PP.routing = 'portfolio' ")
                ->innerJoin('M.keywords', 'K', 'WITH', "K.keyword = 'showcase' ")
                ->leftJoin('M.translations', 'T', 'WITH', "T.locale = :locale")
                ->setParameter('locale', $locale);
    ...
    public function portfolioAction()
    {
        ...
        $queryBuilder = $em->getRepository('ItcAdminBundle:Keyword\Keyword')
                ->createQueryBuilder('M')
                ->select('M, T')
                ->innerJoin('M.menus', 'G', 'WITH', "G.parent_id IN (" . implode(',', $list) . ") ")
                ->leftJoin('M.translations', 'T', 'WITH', "T.locale = :locale")
...
//        $entity = $queryBuilder->getQuery()->getOneOrNullResult();
    ...
    public function otherAction($translit)
    {
        ...
        if ($entity === NULL) {
            ...
            $httpKernel = $this->container->get('http_kernel');
            $controller = $this->getController('onenews');
            $res = $httpKernel->forward($controller, array(
                "translit" => $translit,
                    ));
        }*/ else {
    ...
    public function rightblockAction( $entity)
    {
        ...
        return array(
            ...
            'entity' => $entity,
            'keywords' => $keywords,
            'menus' => $menus, //$entity->getChildren(),
    ...
    public function sendMailAction($translit, Request $request)
    {
        ...
        if ($form->isValid()) {
            $body = $this->renderView('MainSiteBundle:Default:sendMail.txt.twig', array('text' => "Пользователь " . $data['fio'] . " email:" . $data['email'] . ".Телефон:" . $data['telefon'] . "Оставил сообщение:" . $data['body']));
